...
int main(const int argc, const char **argv)
{
    ...
    float mem = (size*5*4+(badmm_mt->m+badmm_mt->n)*3*4
                 +gpu_info.MAX_GRID_SIZE*gpu_info.MAX_BLOCK_SIZE*2*4)/pow(2,30);
    ...
    printf("rho = %f, Max_Iteration = %d, tol = %f, print every %d steps, save result: %d\n",
           badmm_para->rho, badmm_para->MAX_ITER,
           badmm_para->tol, badmm_mt->print_step, badmm_mt->SAVEFILE);
...
void matInit(float* &X, unsigned int size, float value)
{
  ...
  void badmm_getGPUInfo(GPUInfo* gpu_info) {
    cudaDeviceProp prop;
    cudaGetDeviceProperties(&prop,0);       // default device
